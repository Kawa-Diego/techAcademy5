...
export class ProductService {
  ...
  public async listPublic(
    ...
    const ctx = { catalogProductCount };
    const data = rows.map((r) => toPublicProduct(r, ctx));
    return buildPaginated(data, total, pagination.page, pagination.pageSize);
  }
  ...
  public async getByIdPublic(id: string): Promise<PublicProduct> {
    ...
    if (!row) throw new AppError(404, 'Produto não encontrado');
    return toPublicProduct(row, { catalogProductCount });
  }
